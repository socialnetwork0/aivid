...
from aivid.utils.url_parser import parse_tiktok_url

from .base import BaseDownloader, DownloadError


class TikTokDownloader(BaseDownloader):
    """Download videos from TikTok using yt-dlp.

    Requires: pip install yt-dlp
    """

    name: ClassVar[str] = "tiktok"
    platform: ClassVar[SourcePlatform] = SourcePlatform.TIKTOK
    ...
    def download(self, url: str, output_dir: str | None = None) -> tuple[str, SourceInfo]:
        ...
        if not video_id:
            raise DownloadError(f"Invalid TikTok URL: {url}")

        config = get_config()
        output_dir = output_dir or config.download.temp_dir or tempfile.gettempdir()
        output_template = f"{output_dir}/tiktok_%(id)s.%(ext)s"
        ...
        try:
            with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                ...
                # TikTok provides video_id in different format
                actual_video_id = info.get("id") or video_id
                ...
                source_info = SourceInfo(
                    ...
                    upload_date=self._parse_timestamp(info.get("timestamp")),
            ...
            raise DownloadError(f"TikTok download failed: {e}") from e
